...
直接提取的 BetterGI 代码及其衍生修改按 GPL-3.0 管理。仓库保留来源、许可证、版权和第三方组件说明。进程拆分有助于保持两个程序的工程边界，但正式发布前仍应完成一次许可证审查。
...
→ 复制 Worker 输出到 plugin/.../worker/win-x64
→ 生成文件哈希清单
...
- GitHub 仓库及本地目录名：`akasha-automation`。
