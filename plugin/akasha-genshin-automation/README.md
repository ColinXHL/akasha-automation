...
默认按 `F9` 切换自动拾取、按 `F12` 切换自动剧情，这两个快捷键都可以在插件设置中修改。快捷键切换结果会保存到当前 Profile 的插件配置，并通过 OSD 显示启用、关闭或失败状态。
...
脚本会构建相邻的 `AkashaNavigator` 仓库，并把插件和 Worker 发布到 Navigator 的 Debug 输出。随后启动脚本打印的 `AkashaNavigator.exe`，在插件中心安装“Akasha 原神自动化”。新版内置原神 Profile 会自动关联此插件；已有 Profile 也可以在已安装插件列表中手动添加。
...
4. 安装新版内置原神 Profile，或在“已安装插件”中把插件添加到已有原神 Profile。
